...
const solutions: Solution[] = [
  { title: "Estate-wide monitoring", description: "Use repeat satellite observations to track broad change across the estate and build a continuous monitoring record.", image: "palmwatch-hero.webp", alt: "Aerial view across a mapped oil-palm estate", tags: ["Satellite context", "Time series"] },
  { title: "Closer risk mapping", description: "Send drones to priority blocks for higher-resolution RGB, multispectral, and thermal evidence that can be matched with satellite trends.", image: "palmwatch-drone.webp", alt: "Mapping drone surveying oil palms", tags: ["Drone survey", "Priority"] },
  { title: "Field action & traceability", description: "Move from a flagged palm to inspection, confirmation, follow-up, and an auditable record.", image: "palmwatch-field-verification.webp", alt: "Agronomists verifying an oil-palm observation in the field", tags: ["Assignments", "Audit trail"] },
];

const services: Service[] = [
  { title: "Satellite estate watch", description: "Track estate-wide change through repeat satellite observations, weather, and available soil-context layers.", image: "palmwatch-hero.webp", alt: "Aerial view of an oil-palm estate arranged in mapped blocks" },
  { title: "Targeted drone survey", description: "Fly priority blocks for higher-resolution RGB, multispectral, and thermal data, then compare it with satellite trends.", image: "palmwatch-drone.webp", alt: "Drone collecting calibrated imagery over mature oil palms" },
  { title: "Cross-scale screening", description: "Compare satellite trends, drone measurements, palm history, and neighbours to identify persistent change.", image: "palmwatch-hero.webp", alt: "Oil-palm rows viewed from above for comparative screening" },
  { title: "Field verification", description: "Send prioritised observations to agronomists for inspection and, where needed, laboratory confirmation.", image: "palmwatch-field-verification.webp", alt: "Field specialists examining a mature oil palm with a tablet" },
  { title: "Monitoring & reporting", description: "Keep satellite updates, drone follow-ups, field findings, and intervention outcomes in one traceable estate record.", image: "palmwatch-drone.webp", alt: "Drone survey providing repeat monitoring evidence for an estate" },
];
...
const features: Feature[] = [
  ...
  { title: "Multisignal health intelligence", description: "No single index is treated as a diagnosis.", detail: "Satellite trends, drone measurements, temporal change, and field evidence are reviewed together.", icon: Layers3, visual: "signals" },
...
const navItems = [["Overview", "#overview"], ["Solutions", "#solutions"], ["Services", "#services"], ["Features", "#features"], ["Journal", "/blog/"]] as const;

function Brand() {
  return <span className="brand"><span className="brand-mark" aria-hidden="true"><svg viewBox="0 0 42 42"><path d="M11 29c3-10 10-16 21-18-1 10-7 18-18 20"/><path className="vein" d="M14 29c4-5 9-9 16-14"/></svg></span><span className="brand-text" aria-hidden="true">PalmWatch</span></span>;
}
  ...
  return <>
    ...
    <main id="main">
      <section className="hero" id="home" aria-labelledby="hero-title">
        <img className="hero-image" src={asset("palmwatch-hero.webp")} alt="Aerial view of a mature oil-palm plantation at sunrise" fetchPriority="high" decoding="async" />
        ...
        <div className="hero-inner">
          ...
          <article className="hero-card" aria-label="Estate-level monitoring"><img src={asset("palmwatch-drone.webp")} alt="Mapping drone surveying oil palms" decoding="async" /><h2>Your entire oil-palm estate. One dashboard.</h2></article>
          <div className="hero-intro"><p>Combine frequent satellite monitoring with targeted drone surveys and field confirmation—estate-wide context with a closer view where it matters most.</p><Button asChild><a href="#services">Explore the workflow <ArrowDown size={17} aria-hidden="true" /></a></Button></div>
        </div>
      </section>

      <section className="statement section" id="overview">
        <p className="section-label">PalmWatch overview</p>
        <div className="statement-copy"><h2>From plantation-scale imagery to a decision for every tree.</h2><p>PalmWatch pairs repeat satellite observations with closer drone measurements. Satellite, weather, and available soil-context layers reveal broad change; targeted drone surveys add higher-resolution evidence before expert field inspection.</p></div>
      ...
      <section className="service-story" id="services" aria-labelledby="services-title">
        <div className="service-sticky">
          <div className="service-backgrounds" aria-hidden="true">{services.map((service, index) => <img className={index === activeService ? "active" : ""} key={service.title} src={asset(service.image)} alt="" loading="lazy" decoding="async" />)}</div><div className="service-shade"/>
          <div className="service-stage">
            ...
            <article className="service-card" key={currentService.title}><img src={asset(currentService.image)} alt={currentService.alt} loading="lazy" decoding="async"/><div><h3>{currentService.title}</h3><p>{currentService.description}</p></div></article>
        ...
        <div className="service-mobile-list"><div className="service-mobile-head"><p className="section-label">Our services</p><h2>Evidence from the air. Decisions in the field.</h2></div>{services.map((service, index) => <article key={service.title}><img src={asset(service.image)} alt={service.alt} loading="lazy" decoding="async"/><span>0{index + 1}/05</span><h3>{service.title}</h3><p>{service.description}</p></article>)}</div>
      </section>
      ...
      <section className="research section" id="research"><div className="research-visual"><img src={asset("palmwatch-field-verification.webp")} alt="Agronomists verifying an oil-palm observation in the field" loading="lazy" decoding="async"/><div className="floating-reading reading-one"><span>NDRE trend</span><strong>−8.4%</strong><small>needs review</small></div><div className="floating-reading reading-two"><span>Palm ID</span><strong>PW-2048</strong><small>Block 06</small></div></div><div className="research-copy"><p className="section-label">Whole-plantation research</p><h2>Health is more than one disease.</h2><p>The expanded NBL research scope evaluates the context around every tree so the model can distinguish possible disease from water, nutrient, pest, and management stress.</p><div className="factor-cloud" aria-label="Research factors"><span>Soil condition</span><span>Leaf condition</span><span>Seed variety</span><span>Geography</span><span>Irrigation</span><span>Fertilizer</span><span>Infestation</span><span>Intercropping</span><span>Adjacent farming</span><span>Plantation age</span></div></div></section>

      <section className="evidence section"><div className="section-head"><div><p className="section-label">Evidence architecture</p><h2>Signals become useful when they agree.</h2></div><p>No universal NDVI threshold can diagnose a palm. PalmWatch compares broad satellite trends with calibrated drone measurements, temporal change, neighbouring palms, and confirmed field labels.</p></div><div className="evidence-grid"><article className="evidence-card"><span>01</span><h3>Spectral</h3><p>Satellite trends plus drone-based NDVI, NDRE, red edge, NIR, and calibrated reflectance.</p></article><article className="evidence-card"><span>02</span><h3>Structural</h3><p>Crown area, density, symmetry, gaps, yellowing, and visible change.</p></article><article className="evidence-card"><span>03</span><h3>Thermal</h3><p>Canopy temperature, transpiration stress, and neighbour-relative anomalies.</p></article><article className="evidence-card"><span>04</span><h3>Contextual</h3><p>Weather, available soil context, age, seed, management history, and spatial clustering.</p></article></div></section>

      <section className="insights section" aria-labelledby="insights-title"><div className="insights-head"><div><p className="section-label">Research journal</p><h2 id="insights-title">Read the evidence behind the map.</h2></div><p>Plain-language, cited explanations of disease risk, remote sensing, field confirmation, and the limits that keep screening honest.</p></div><div className="insights-list"><a href="/blog/ganoderma-basal-stem-rot-oil-palm/"><span>Ganoderma · 9 min</span><h3>What teams can see—and what they cannot</h3><p>Symptoms, progression, confirmation, and the role of repeat observation.</p><strong>Read article →</strong></a><a href="/blog/ganoderma-yield-loss-per-acre/"><span>Yield loss · 10 min</span><h3>How much can Ganoderma cost per acre?</h3><p>A transparent calculation built from estate-specific assumptions.</p><strong>Use the calculator →</strong></a><a href="/blog/satellite-monitoring-oil-palm-estates/"><span>Satellite monitoring · 9 min</span><h3>Continuous context, honest limits</h3><p>What repeat observations can reveal before closer surveys begin.</p><strong>Read article →</strong></a></div><div className="insights-actions"><a className="button button-lime" href="/blog/">Browse the journal</a><a className="text-link" href="/white-paper/">Read White Paper v2 →</a></div></section>
