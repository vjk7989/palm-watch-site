...
function FeatureVisual({ type }: { type: Feature["visual"] }) {
  if (type === "identity") return <div className="identity-visual" aria-hidden="true"><span>PW-2048</span><i/><i/><i/><b>2024</b><b>2025</b><b>Now</b></div>;
  if (type === "signals") return <div className="signal-visual" aria-hidden="true"><span style={{ "--level": "82%" } as CSSProperties}>Spectral</span><span style={{ "--level": "64%" } as CSSProperties}>Structural</span><span style={{ "--level": "48%" } as CSSProperties}>Thermal</span><span style={{ "--level": "71%" } as CSSProperties}>Context</span></div>;
  if (type === "estate") return <div className="estate-visual" aria-hidden="true">{Array.from({ length: 28 }, (_, index) => <i className={index === 9 || index === 17 ? "review" : ""} key={index}/>)}</div>;
  return <div className="loop-visual" aria-hidden="true"><span><ScanLine/>Observe</span><span><ClipboardCheck/>Confirm</span><span><RefreshCw/>Learn</span></div>;
}
    ...
    <main id="main">
      <section className="hero" id="home" aria-labelledby="hero-title">
        ...
        <div className="hero-inner">
          <div className="hero-copy"><span className="hero-kicker">AI-assisted oil-palm intelligence</span><h1 id="hero-title">See the health of every palm, sooner.</h1></div>
